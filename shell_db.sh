...
NOME=$1
#read NOME
QUERY="SELECT cod, Nome, email FROM $TABELA WHERE Nome like '$NOME%'"
RESP=`sqlite3 $DB "$QUERY"`
case $? in
...
deleta_dados(){
	echo "Informe o nome do cliente que deseja apagar"
	read NOME

	QUERY="DELETE FROM $TABELA WHERE Nome like '$NOME%'"
RESP=`sqlite3 $DB "$QUERY"`
case $? in
